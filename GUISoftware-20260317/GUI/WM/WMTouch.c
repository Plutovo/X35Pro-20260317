...
/**********************************************************************************/
//return 0: no valid area touched
//       1: valid area touched
/**********************************************************************************/
int WM_InquireTouchState(void)
{
    int r = 0;
    WM_CRITICAL_HANDLE CHWin;
    GUI_PID_STATE State, StateNew;
...
//
/**********************************************************************************/
WM_HWIN WM_InquireTouchHandle(void)
{
    WM_HWIN hWin = 0;
    WM_CRITICAL_HANDLE CHWin;
    GUI_PID_STATE StateNew;
    static GUI_PID_STATE LastState;
    GUI_PID_GetState(&StateNew);
    WM_LOCK();
    WM__AddCriticalHandle(&CHWin);
    if ((LastState.x != StateNew.x) || (LastState.y != StateNew.y) || (LastState.Pressed != StateNew.Pressed))
    {
        CHWin.hWin = _Screen2Win(&StateNew);
        if (WM__IsInModalArea(CHWin.hWin))
        {

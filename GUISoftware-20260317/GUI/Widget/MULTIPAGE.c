...
#define MULTIPAGE_FONT_DEFAULT        &GUI_Font_16
#endif
#ifndef MULTIPAGE_ALIGN_DEFAULT
#define MULTIPAGE_ALIGN_DEFAULT       (MULTIPAGE_ALIGN_RIGHT | MULTIPAGE_ALIGN_TOP)
#endif
/* Define colors */
#define MULTIPAGE_NUMCOLORS 2
#ifndef MULTIPAGE_BKCOLOR0_DEFAULT
#define MULTIPAGE_BKCOLOR0_DEFAULT    0xA0A0A0//0x6E6450 /* disabled page */
#endif
#ifndef MULTIPAGE_BKCOLOR1_DEFAULT
#define MULTIPAGE_BKCOLOR1_DEFAULT    GUI_YAN_BLUE//0xFFE2BF//0x6E6450 /* enabled page */
#endif
...
static void _CalcClientRect(MULTIPAGE_Obj* pObj, GUI_RECT* pRect)
{
    ...
    {
        pRect->y1 -= GUI_GetYSizeOfFont(pObj->Font) + 12;
    }
    else
    {
        pRect->y0 += GUI_GetYSizeOfFont(pObj->Font) + 12;
    }
}
/***************************************************************************************************
*
...
{
    return _GetPagePosX(pObj, pObj->Handles.NumItems);//+40
}
/***************************************************************************************************
*
*       _GetTextRect
...
static void _GetTextRect(MULTIPAGE_Obj* pObj, GUI_RECT* pRect)
{
    ...
    int Width, Height;
    //int offset = 80;
    Height = GUI_GetYSizeOfFont(pObj->Font) + 12;
    ...
    {
        Width = _GetTextWidth(pObj) + 40;//+40
    }
                          ...
                          const GUI_RECT* pRect, int x0, int w, int ColorIndex)
{
    ...
    if (pObj->Selection == Index)
    {
        if (pObj->Align & MULTIPAGE_ALIGN_BOTTOM)
        {
            ...
            Arc(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1 - 6,5,6,0x60);
            LCD_SetColor(GUI_WHITE);
            Arc(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1 - 6,5,6,0x1E);
            //Bevel(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1 - 6,6);
            LCD_SetBkColor(pObj->aBkColor[ColorIndex]);
        }
        else
        {
            LCD_SetColor(0x222222);
            Arc(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1,5,6,0xE1);
            LCD_SetColor(GUI_WHITE);
            Arc(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1,5,6,0x06);
            LCD_SetColor(0x808080);
            Bevel(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1,6);
            LCD_SetColor(pObj->aBkColor[ColorIndex]);
            GUI_DrawHLine(r.y1, r.x0+2, r.x1 - 3);
            GUI_DrawHLine(r.y1+1, r.x0+2, r.x1 - 3);
            LCD_SetBkColor(pObj->aBkColor[ColorIndex]);
        }
    }
    else
    {
        LCD_SetColor(GUI_WHITE);
        Arc(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1 - 6,5,6,0xE1);
        LCD_SetColor(0x222222);
        Arc(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1 - 6,5,6,0x1E);
        Bevel(r.x0 + 6, r.y0 + 6, r.x1 - 6, r.y1 - 6,6);
        LCD_SetBkColor(pObj->aBkColor[ColorIndex]);
    ...
    GUI_DispStringAt(pText, r.x0 + 4 + 10, pRect->y0 + 6);//+10
}
/***************************************************************************************************
*
...
{
    GUI_RECT rBorder,rClient;
    //GUI_GetClientRect(&rClient);
    //pObj->Widget.pEffect->EffectSize = 1;
// LCD_SetColor(GUI_WHITE);
// Arc(rClient.x0 + 8, rClient.y0 + 8, rClient.x1 - 8, rClient.y1,7,8,0xFF);
// LCD_SetColor(0x222222);
// Arc(rClient.x0 + 8, rClient.y0 + 8, rClient.x1 - 8, rClient.y1,7,8,0x1E);
// Bevel(rClient.x0 + 8, rClient.y0 + 8, rClient.x1 - 8, rClient.y1,7);
...
// FillBevel(rClient.x0 +8,rClient.y0+8,rClient.x1-8,rClient.y1-8,6);
    ...
    WIDGET__EFFECT_DrawDownRect(&pObj->Widget, &rBorder);
    //LCD_SetColor(0x555555);
    //Arc(rBorder.x0, rBorder.y0, rBorder.x1, rBorder.y1,-3,0,0xE1);
    //
    //Arc(rBorder.x0, rBorder.y0, rBorder.x1, rBorder.y1,-3,0,0x1E);
    LCD_SetColor(GUI_WHITE);
    //Bevel(rBorder.x0, rBorder.y0+1, rBorder.x1, rBorder.y1,0);
    //LCD_SetColor(GUI_WHITE);
    //Arc(rBorder.x0, rBorder.y0 + 1, rBorder.x1, rBorder.y1 ,-3,0,0x1E);
    //LCD_SetColor(0x555555);
    //Arc(rBorder.x0, rBorder.y0 + 2, rBorder.x1, rBorder.y1,-3,0,0xE1);
    //Bevel(rBorder.x0, rBorder.y0 +1, rBorder.x1, rBorder.y1+2,0);
    /* Draw text items */
    if (pObj->Handles.NumItems > 0)
    {
        ...
        for (i = 0; i < pObj->Handles.NumItems; i++)
        {
            ...
            x0 += w;
            w   = GUI_GetStringDistX(&pPage->acText) + 10 + 10;//+10
...
*       _OnTouch
*/
static int _OnTouch(MULTIPAGE_Handle hObj, MULTIPAGE_Obj* pObj, WM_MESSAGE*pMsg)
{
    GUI_PID_STATE* pState;
    int Notification = 0;
    if(pObj->PageChangeEnable)
    {
...
static void _Callback (WM_MESSAGE *pMsg)
{
    ...
    case WM_PAINT:
        ...
        break;
    case WM_TOUCH:
        //if(_OnTouch(hObj, pObj, pMsg)==WM_NOTIFICATION_CLICKED)
        //{
        //TouchTaskHandle(hObj);
        //}
    ...
    case WM_NOTIFY_PARENT_REFLECTION:
        WM_SendMessageNoPara(MULTIPAGE_GetWindow(hObj,MULTIPAGE_GetSelection(hObj)),WM_INIT_DIALOG);
        break;
    default:

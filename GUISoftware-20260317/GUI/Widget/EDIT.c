...
#define EDIT_FONT_DEFAULT &GUI_Font_15
#endif
...
static void _Paint(EDIT_Obj* pObj, EDIT_Handle hObj)
{
    ...
    WM_ITERATE_START(NULL)
    {
        ...
        if (pObj->Widget.State & WIDGET_STATE_FOCUS)
        {
			GUI_SetColor(GUI_BLACK);//游标颜色
			GUI_FillRect(rInvert.x0, rInvert.y0, rInvert.x0 + CursorWidth - 1, rInvert.y1);
...
static void _OnTouch(EDIT_Handle hObj, EDIT_Obj* pObj, WM_MESSAGE*pMsg)
{
    ...
    if (pMsg->Data.p)    /* Something happened in our area (pressed or released) */
    {
        if (pState->Pressed)
        {
			...
			{
				WM_NotifyParent(hObj, WM_NOTIFICATION_CLICKED);//发送点击消息 
			}  
        }
        else
        {

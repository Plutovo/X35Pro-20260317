...
// 	else
// 	{
// 		WIDGET__EFFECT_DrawDownRect(&pObj->Widget, &r);
// 	}
    //WIDGET__EFFECT_DrawUpRect(&pObj->Widget, &r);
	//WIDGET__EFFECT_DrawDownRect(&pObj->Widget, &r);
    ...
    */
    if (Pos.x1_RightArrow != Pos.x1)
    {
        ...
        //LCD_SetColor(pObj->aColor[0]);
		WIDGET__FillRectEx(&pObj->Widget, &r);
		if(pObj->Widget.State&SCROLLBAR_CF_FOCUSSABLE)
		{
...
*       SCROLLBAR_IsPressed
*/
unsigned SCROLLBAR_IsPressed(SCROLLBAR_Handle hObj)
{
    unsigned r = 0;
    if (hObj)
    {
        ...
        WM_LOCK();
        pObj = SCROLLBAR_H2P(hObj);
        r = (pObj->Widget.State & SCROLLBAR_STATE_PRESSED) ? 1 : 0;
        WM_UNLOCK();
    }
    return r;
}
/***************************************************************************************************
*
*       SCROLLBAR_SetHandleState
*/
void SCROLLBAR_SetHandleState(SCROLLBAR_Handle hObj, int State)
{
...
*       SCROLLBAR_SetHandleState
*/
void SCROLLBAR_SetArrow(SCROLLBAR_Handle hObj, int arror)
{
	SCROLLBAR_Obj* pObj;
	if(hObj)
	{

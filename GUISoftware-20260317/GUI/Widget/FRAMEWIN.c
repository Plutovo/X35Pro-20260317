...
//#define FRAMEWIN_BARCOLOR_ACTIVE_DEFAULT 0x333333//对话框标题栏颜色
//#else
#define FRAMEWIN_BARCOLOR_ACTIVE_DEFAULT GUI_YAN_BLUE//0xD05356
//#endif
#endif
/* Default barcolor when framewin is inactive */
#ifndef FRAMEWIN_BARCOLOR_INACTIVE_DEFAULT
#define FRAMEWIN_BARCOLOR_INACTIVE_DEFAULT GUI_YAN_BLUE
#endif
/* Default framecolor */
...
static void _Paint(FRAMEWIN_Obj* pObj)
{
	...
	{
		/* Draw Title */
		if(pObj->TitleStyle == 1)
		{
			LCD_SetBkColor(0x333333);
		...
		if (pObj->Props.BorderSize >= 2)
		{
			...
			//{
				...
				Arc(r1.x0+8, r1.y0+8, r1.x1-8, r1.y1,7,8,0xE1);		
				LCD_SetColor(0x313131);
				Arc(r1.x0+8, r1.y0+8, r1.x1-8, r1.y1 ,7,8,0x1E);		
				Bevel(r1.x0+8, r1.y0+8, r1.x1-8, r1.y1,8);
				GUI_DrawHLine(r1.y1,r1.x0,r1.x1);
			//}
			//else 
			//{
			//	LCD_SetColor(0x420606);
			//	GUI_DrawHLine(r.y1-1,r.x0,r.x1);
			//	GUI_DrawHLine(r.y1,r.x0,r.x1);
			//}
		}
		GUI_DRAW__Draw(pObj->ahDrawObj[0], 0, 0);	
		GUI_DRAW__Draw(pObj->ahDrawObj[1], 0, 0);	
	}
	WM_ITERATE_END();
}
/***************************************************************************************************
...
static void _FRAMEWIN_Callback (WM_MESSAGE *pMsg)
{
    ...
    case WM_DELETE:
		...
		GUI_ALLOC_FreePtr(&pObj->ahDrawObj[0]);//内存没有释放
...
*      
*/
void FRAMEWIN_SetTitleStyle(FRAMEWIN_Handle hObj, int Style)
{
	if (hObj)
	{
		FRAMEWIN_Obj* pObj;
		GUI_LOCK();
		pObj = FRAMEWIN_H2P(hObj);
		pObj->TitleStyle = Style;
		GUI_UNLOCK();
	}
}

void FRAMEWIN__SetDrawObj(FRAMEWIN_Handle hObj, GUI_DRAW_HANDLE hDrawObj,unsigned char Index)
{
	if (hObj)
	{
		FRAMEWIN_Obj* pObj;
		GUI_LOCK();
		pObj = FRAMEWIN_H2P(hObj);
		FRAMEWIN_ASSERT_IS_VALID_PTR(pObj);
...
void FRAMEWIN_SetBitmapEx(FRAMEWIN_Handle hObj, const GUI_BITMAP * pBitmap, int x, int y,unsigned char Index)
{
	FRAMEWIN__SetDrawObj(hObj, GUI_DRAW_BITMAP_Create(pBitmap, x, y),Index);
}

void FRAMEWIN_SetBitmap(FRAMEWIN_Handle hObj, const GUI_BITMAP* pBitmap, int x, int y,unsigned char Index)
{
	FRAMEWIN_SetBitmapEx(hObj, pBitmap, x, y, Index);
}

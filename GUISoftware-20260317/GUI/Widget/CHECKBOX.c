...
#define CHECKBOX_FONT_DEFAULT &GUI_Font_16
#endif
...
#define CHECKBOX_BKCOLOR_DEFAULT GUI_YAN_BLUE//0x6E6450           /* Text background color */
#endif
#ifndef CHECKBOX_SPACING_DEFAULT
#define CHECKBOX_SPACING_DEFAULT 6
#endif
...
static void _Paint(CHECKBOX_Obj* pObj, CHECKBOX_Handle hObj)
{
    ...
    RectBox.x1 = pObj->Props.apBm[0]->XSize-1+offset;
    RectBox.y1 = pObj->Props.apBm[0]->YSize-1+offset;
    r.x0 = 3;
    r.y0 = 3;
    r.x1 = RectBox.x1+3;
    r.y1 = RectBox.y1+3;
    WM_SetUserClipRect(&r);
    LCD_SetColor(pObj->Props.aBkColorBox[ColorIndex]);
    FillBevel(r.x0 +8,r.y0+8,r.x1-8,r.y1-8,8);
    _WIDGET_EFFECT_3D_DrawDownRoundRect(&r);
    RectBox.x0 = offset;
    RectBox.y0 = offset;
    ...
    if (pObj->CurrentState)
    {
        ...
        GUI_DrawBitmap(pObj->Props.apBm[Index],RectBox.x0 , RectBox.y0);
    }
    ...
    if (pObj->hpText)
    {
        ...
        if (pObj->Widget.State & WIDGET_STATE_FOCUS)
        {
            ...
            WIDGET__DrawFocusRect(&pObj->Widget, &RectFocus, 0); //text 外围的虚线框
        }
#endif
    }
}
...
static void _Delete(CHECKBOX_Obj* pObj)//zkq add 20170120 释放text所占用内存
{
...
static void _CHECKBOX_Callback (WM_MESSAGE *pMsg)
{
    ...
    case WM_TOUCH:
        ...
        break;
    case WM_DELETE://zkq add 20170120 释放text所占用内存
        _Delete(pObj);

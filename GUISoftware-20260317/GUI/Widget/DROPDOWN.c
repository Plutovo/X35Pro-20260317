...
#define DROPDOWN_FONT_DEFAULT &GUI_Font_16
#endif
/* Define colors */
...
#define DROPDOWN_BKCOLOR2_DEFAULT 0x4080FF//GUI_BLUE      /* Selected, focus */
#endif
...
#define DROPDOWN_ALIGN_DEFAULT GUI_TA_LEFT|GUI_TA_VCENTER      /* Default text alignment */
#endif
/***************************************************************************************************
*
*       Static data
...
//          DROPDOWN_SetSel(hObj, LISTBOX_GetSel(pObj->hListWin));
			i = LISTBOX_GetSel(pObj->hListWin);
			if(i != -1)
			{
				DROPDOWN_SetSel(hObj, i);
			}
...
/***************************************************************************************************
* Set the number of items in the dropdown list
*       DROPDOWN_GetNumItems  //  20130407
*/
void DROPDOWN_SetNumItems(DROPDOWN_Handle hObj,int NumItems)
{
    DROPDOWN_Obj* pObj;
    if (hObj)
    {
        ...
        pObj = DROPDOWN_H2P(hObj);
        ASSERT_IS_VALID_PTR(pObj);
        pObj->Handles.NumItems = NumItems;
        WM_UNLOCK();
    }
}
/***************************************************************************************************
...
*       DROPDOWN_GetState
*/
int DROPDOWN_GetState(DROPDOWN_Handle hObj)
{
    DROPDOWN_Obj* pObj;
    int r = 0;
    if (hObj)
...
*       DROPDOWN_GetLstWin
*/
int DROPDOWN_GetLstWin(DROPDOWN_Handle hObj)
{
    DROPDOWN_Obj* pObj;
    int r = 0;
    if (hObj)
...
*       DROPDOWN_SetState
*/
void DROPDOWN_SetState(DROPDOWN_Handle hObj,int state)
{
    DROPDOWN_Obj* pObj;
    if (hObj)
    {
...
void DROPDOWN_SetItemDisabled(DROPDOWN_Handle hObj, unsigned Index, int OnOff)
{
	...
	if (hObj)
	{
		WM_LOCK();
		pObj = DROPDOWN_H2P(hObj);
		LISTBOX_SetItemDisabled(pObj->hListWin, Index, OnOff);
		WM_UNLOCK();
	}
}

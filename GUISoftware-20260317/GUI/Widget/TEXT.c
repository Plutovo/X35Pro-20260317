...
#define TEXT_FONT_DEFAULT &GUI_Font_16;
#endif
#ifndef TEXT_DEFAULT_TEXT_COLOR
#define TEXT_DEFAULT_TEXT_COLOR GUI_BLACK
#endif
#ifndef TEXT_WRAPMODE_DEFAULT   //add 20161115 是否自动换行
#define TEXT_WRAPMODE_DEFAULT GUI_WRAPMODE_NONE
#endif
/***************************************************************************************************
*
*       Static data
...
static GUI_WRAPMODE     _DefaultWrapMode = TEXT_WRAPMODE_DEFAULT;
/***************************************************************************************************
*
*       Static routines
*
...
static void _Paint(TEXT_Handle hObj, TEXT_Obj* pObj)
{
    ...
    if (pObj->hpText)
    {
        ...
        GUI_DispStringInRectWrap(s, &Rect, pObj->Align, pObj->WrapMode);//修改 20161115 是否自动换行
    }
}
/***************************************************************************************************
*
                          ...
                          int WinFlags, int ExFlags, int Id, const char* pText)
{
    ...
    if (hObj)
    {
		...
		pObj->WrapMode = TEXT_WRAPMODE_DEFAULT;
        WM_UNLOCK();
    }
    else
    {
...
/*********************************************************************
 * @brief           TEXT_SetTextWrapMode  //add 20161115
 * @param[in]       GUI_WRAPMODE_NONE:No word wrap
 *                  GUI_WRAPMODE_WORD:Word wrap
 *                  GUI_WRAPMODE_CHAR:Char wrap
 * @return          None.
 **********************************************************************/
void TEXT_SetTextWrapMode(TEXT_Handle hObj, GUI_WRAPMODE WrapMode)
{
    if (hObj)
    {
        TEXT_Obj* pObj;
...
void TEXT_SetFrame(TEXT_Handle hObj,GUI_COLOR Color)
{
    if (hObj)
    {
        TEXT_Obj* pObj;
        WM_LOCK();
        pObj = TEXT_H2P(hObj);
        pObj->Align |= (TEXT_CF_LEFT | TEXT_CF_RIGHT | TEXT_CF_TOP | TEXT_CF_BOTTOM);
        pObj->BkColor = Color;
        WM_UNLOCK();
        if(Color != GUI_WHITE)//如果是白色背景，则显示为透明
        {
            WM_ClrHasTrans(hObj);
        }
    }

...
#define LISTBOX_FONT_DEFAULT &GUI_Font_16
#endif
/* Define colors */
#ifndef LISTBOX_BKCOLOR0_DEFAULT
#define LISTBOX_BKCOLOR0_DEFAULT 0xFAEFE6     /* Not selected */
#endif
#ifndef LISTBOX_BKCOLOR1_DEFAULT
#define LISTBOX_BKCOLOR1_DEFAULT GUI_GRAY      /* Selected, no focus */
#endif
#ifndef LISTBOX_BKCOLOR2_DEFAULT
#define LISTBOX_BKCOLOR2_DEFAULT 0x4080FF//GUI_BLUE      /* Selected, focus */
#endif
...
{
    I16P textalign = GUI_Context.TextAlign;
...
//         GUI_DispStringAt(s, pDrawItemInfo->x0 + 1, pDrawItemInfo->y0);
		GUI_DispStringAt(s, pDrawItemInfo->x0 + 1 + pObj->DispOffsetX, pDrawItemInfo->y0+pObj->DispOffsetY);

		GUI_DRAW__Draw(pItem->isShowBitmap?pObj->ahDrawObj:0, pDrawItemInfo->x0 + 1, pDrawItemInfo->y0+pObj->DispOffsetY);
        ...
        GUI_Context.TextAlign = textalign;
        return 0;
    }
    }
    GUI_Context.TextAlign = textalign;
...
static void _OnPaint(LISTBOX_Handle hObj, LISTBOX_Obj* pObj, WM_MESSAGE* pMsg)
{
    ...
    ClipRect = pObj->Widget.Win.InvalidRect;
...
/***************************************************************************************************/
void LISTBOX__SetDrawObj(LISTBOX_Handle hObj, GUI_DRAW_HANDLE hDrawObj)
{
	if (hObj)
	{
		LISTBOX_Obj* pObj;
...
void LISTBOX_SetBitmapEx(LISTBOX_Handle hObj, const GUI_BITMAP * pBitmap, int x, int y)
{
	LISTBOX__SetDrawObj(hObj, GUI_DRAW_BITMAP_Create(pBitmap, x, y));
}

void LISTBOX_SetBitmap(LISTBOX_Handle hObj, const GUI_BITMAP* pBitmap)
{
	LISTBOX_SetBitmapEx(hObj, pBitmap, 0, 0);
}
/***************************************************************************************************/
void LISTBOX_AddStringWithBitmap(LISTBOX_Handle hObj, const char* s, char flag)
{
	if (hObj && s)
	{
		LISTBOX_Obj* pObj;
...
void LISTBOX_SetOff_Y(LISTBOX_Handle hObj,unsigned char off_y)
{
	LISTBOX_Obj* pObj;
	if (hObj)
	{
		WM_LOCK();
		pObj = LISTBOX_H2P(hObj);
		pObj->DispOffsetY = off_y;
		WM_UNLOCK();
	}
}

void LISTBOX_SetOff_X(LISTBOX_Handle hObj,unsigned char off_x)
{
	LISTBOX_Obj* pObj;
	if (hObj)
	{

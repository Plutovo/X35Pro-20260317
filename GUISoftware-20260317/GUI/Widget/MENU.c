...
*/
//#ifndef PROPER_H 
//#include "__Proper.h" //cpp del 20130514 
//#endif
...
#define MENU_FONT_DEFAULT         &GUI_Font_16
#endif
/* Define default effect */
#ifndef MENU_EFFECT_DEFAULT
#define MENU_EFFECT_DEFAULT       &WIDGET_Effect_3D
#endif
...
#define MENU_BKCOLOR0_DEFAULT     GUI_YAN_BLUE//GUI_LIGHTGRAY
#endif
...
#define MENU_BKCOLOR1_DEFAULT     GUI_YELLOW//0x980000
#endif
...
#define MENU_BKCOLOR2_DEFAULT     GUI_YAN_BLUE//GUI_LIGHTGRAY
#endif
...
#define MENU_BKCOLOR3_DEFAULT     GUI_BLUE//0x980000
#endif
...
#define MENU_BKCOLOR4_DEFAULT     GUI_BLACK//0x7C7C7C
#endif
/* Define borders */
...
#define MENU_BORDER_TOP_DEFAULT     4//6//12// //2  //cpp md 20130514
#endif
#ifndef MENU_BORDER_BOTTOM_DEFAULT
#define MENU_BORDER_BOTTOM_DEFAULT  4//6//12// //2 
#endif
/*********************************************************************
*
*       Static data
        ...
        if (pObj->Flags & MENU_SF_VERTICAL)
        {
            ...
            if(pObj->VerticalFlg)
            {
                ...
                for (i = 0; i < NumItems; i++)
                {
                    ...
                    {
                        xPos = (_GetItemWidth(hObj, pObj, i)/2);
...
*       _SelectItem
*/
static int _SelectItem(MENU_Handle hObj, MENU_Obj* pObj, unsigned Index)
{
    int flag = 0;
    MENU_ITEM* pItem;
    pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, Index);
    //if (((pObj->Sel != (int)Index)&((pItem->Flags&MENU_IF_DISABLED)==0))) {
    if ((pItem->Flags&MENU_IF_DISABLED)==0)
    {
        _SetCapture(hObj, pObj);
    ...
    if (XYInWidget)
    {
        ...
        if (ItemIndex >= 0)
        {
            ...
            if (Pressed == 1)
            {
                ...
                if(_SelectItem(hObj, pObj, ItemIndex))
                {
...
static void _OnPaint(MENU_Handle hObj, MENU_Obj* pObj)
{
    ...
    if (pObj->Flags & MENU_SF_VERTICAL)
    {
        ...
        if(pObj->VerticalFlg)
        {
            int ItemHeight, xSize,FillRect_x0;
            xSize = _CalcMenuSizeX(hObj, pObj);
            FillRect.x1 = xSize - EffectSize - 1;
            FillRect_x0 = FillRect.x0;
            for (i = 0; i < NumItems; i++)
            {
                if(i %2)
                {
                    ...
                    FillRect.x0 = FillRect_x0 + xSize/2;
...
**********************************************************************
*/
//zkq add 20170119 新增获取menu状态标识函数 
/***************************************************************************************************
*       MENU__GetItemFlags
*       
*       返回值：0：开启 1：禁止
*/
unsigned char MENU__GetItemFlags(MENU_Obj* pObj, unsigned Index)
{
    MENU_ITEM* pItem = (MENU_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, Index);
    unsigned char Flags;
    Flags = pItem->Flags;
...
*       MENU__GetSel
*/
int MENU_GetSel(MENU_Handle hObj)
{
    int r = -1;
    MENU_Obj* pObj;
    WM_LOCK();
...
*       MENU_AddItem
*/
void MENU_SetVerticalFlg(MENU_Handle hObj, int VerticalFlg)
{
    if(hObj)
    {
        MENU_Obj *pObj;

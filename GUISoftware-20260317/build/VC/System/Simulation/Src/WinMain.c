  ...
  case WM_PAINT:
    hdc = BeginPaint(hWnd, &ps); {
      HDC hdcImage = CreateCompatibleDC(hdc); {
        ...
        BitBlt(hdc, 0, 0, bmpDevice.bmWidth, bmpDevice.bmHeight, hdcImage, 0, 0, SRCCOPY);
        } DeleteDC(hdcImage);
    } EndPaint(hWnd, &ps);
    break;
  case WM_DESTROY:
      ...
      HDC hdcImage = CreateCompatibleDC(hdc); {
        if (Index < countof(_aHardkey)) {
          ...
          SelectObject(hdcImage, _ahBmpDevice[_aHardkey[Index].IsPressed]);
			BitBlt(hdc, 0, 0, bmpDevice.bmWidth, bmpDevice.bmHeight, hdcImage, _aHardkey[Index].x0, _aHardkey[Index].y0, SRCCOPY);
        }
      } DeleteDC(hdcImage);
    } EndPaint(hWnd, &ps);
    break;
...
static int _WinMain_NoClean(HINSTANCE hInstance, 
  ...
  if (_ahBmpDevice[0] ==0)
    _ahBmpDevice[0] = (HBITMAP)LoadImage(_hInst, (LPCTSTR) IDB_BITMAP1, IMAGE_BITMAP,  0, 0, 0);
  if (_ahBmpDevice[1] ==0)
    _ahBmpDevice[1] = (HBITMAP)LoadImage(_hInst, (LPCTSTR) IDB_BITMAP2, IMAGE_BITMAP,  0, 0, 0);

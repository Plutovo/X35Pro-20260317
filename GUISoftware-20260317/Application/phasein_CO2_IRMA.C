...
extern Int16U phaseinAlarm;                 //phasein 技术报警标志
...
IM_ALL_DATA      * pAll_Data = NULL;            //指向用户区数据，增加定义时赋初值
static IM_ALL_DATA UserDataMem;                //供用户使用的数据的空间
static struct IRMA_RX_CTRL Irma_RX_Ctrl;      //数据接收控制字
static struct IRMA_RX_FRAME Irma_RX_Frame[8]; //接收缓冲区，采用队列结构
...
static enum IM_FUNC_STS Irma_setOnOff(enum IM_FUNC_STS onoff);  //该函数转静态
int Im_getSlowData(enum IM_GET_DATA FirstPara, unsigned char SecondPara);   //该函数转静态
void Module_Alarm_Judge(void);              //phasein技术报警判断函数
static enum IM_FUNC_STS  Irma_getOnOff(void);                   //该函数转静态
////////////////////////////////////////////////////////////////////////////////
unsigned int PhaseInIRMA_ConnFlag = 0;//检测pHasein是否连接的计时标志
//【函数区】
...
#define RTNSTAT      Irma_RX_Ctrl.R_rtnstat
    ...
    PhaseInIRMA_ConnFlag = 0;//超时计数清零
	pHaseinMoudleCheck = 0;//yaoyao add 20220707 连接pHasein是否连接标志置位
	switch(NOWBYTE)
    {
    case 1://IRMA_RX_FLG2
    ...
    case 2://IRMA_RX_ID
            ...
            //接收新数据, 否则返回NOWBYTE0放弃此帧
        {
            ...
            //STATE0 &= 0x80;
            //STATE0  |= (RxByte + 1); //change the Irma_RX_Ctrl.R_state[0] here
			state  = RxByte + 1; //change the Irma_RX_Ctrl.R_state[0] here   麻醉气体
            NOWBYTE = 3;
            RTNSTAT = IM_BUSY;
        }
...
// 		if( Masimo_EtCO2_Config_On && ((AA_Module_Flag.EtCO2ModuleExist&0x02) != 2) )//选配开通 未置位  //ylk modify 20181128
// 		{
// 			AA_Module_Flag.EtCO2ModuleExist |= 1<<1;   //djq add 2013-12-23 第二位置1
// 		}
// 		else if( MasimoISA_EtCO2_Config_On && ((AA_Module_Flag.EtCO2ModuleExist&0x04) != 4) )//选配开通 未置位
// 		{
// 			AA_Module_Flag.EtCO2ModuleExist |= 1<<2;   //djq add 2013-12-23 第二位置1
// 		}
        ...
        if(CHKSUM==0) //Data OK
        {
            ...
            default:
                ...
                if(STATE1 >= 90) //0+1+..+9=45
                {
					...
					Irma_setOnOff(IM_ON);//【设置状态到onoff】
                    COMM_TMOUT = 0;      //通讯恢复正常
                    //printf("\n\n\n\n\n\n              Have set IM_ON\n");
                }
                break;
...
#define SLOW_DATA5   (Irma_RX_Frame[IM_RX_HEAD].slow_data[5])
    ...
    {
		pAll_Data->slow.GenVal.O2_Calib = (SUMMARY_STAT&(1<<7))>>7;//需要氧气量程校准 20180205 add
        switch (Irma_RX_Frame[IM_RX_HEAD].id)  //将case条件中的数值换成枚举常量
        {
        case IRMA_INSP_VALS: //frame0: InspVals
			if(SLOW_DATA0 != 0xFF)//气道适配器移除，数据为0xFF，应该滤掉。
            {
            	pAll_Data->slow.InspVal[IM_GAS_CO2] = SLOW_DATA0;
			}
            if(SLOW_DATA1 != 0xFF)//if(SLOW_DATA1 <= 100)
            {
                pAll_Data->slow.InspVal[IM_GAS_N2O] = SLOW_DATA1;
            }
			if(SLOW_DATA2 != 0xFF)
            {
                if(pAll_Data->slow.GenVal.ax1_id)
                {
            		pAll_Data->slow.InspVal[IM_GAS_AX1] = SLOW_DATA2;
				}
				else
                {
                    pAll_Data->slow.InspVal[IM_GAS_AX1] = 0;
                }
            }
			if(SLOW_DATA3 != 0xFF)
            {
                if(pAll_Data->slow.GenVal.ax2_id)
                {
            		pAll_Data->slow.InspVal[IM_GAS_AX2] = SLOW_DATA3;
				}
                else
                {
                    pAll_Data->slow.InspVal[IM_GAS_AX2] = 0;
                }
            }
        ...
        case IRMA_EXP_VALS: //frame1: ExpVals
			...
			if(SLOW_DATA2 != 0xFF)
            {
                ...
                {
                    pAll_Data->slow.ExpVal[IM_GAS_AX1] = 0;
                }
            }
            if(SLOW_DATA3 != 0xFF)
            {
                ...
                {
                    pAll_Data->slow.ExpVal[IM_GAS_AX2] = 0;
                }
            }
        ...
        case IRMA_MOM_VALS: //frame2: MomentVals
            ...
            {
                pAll_Data->slow.MomtVal[IM_GAS_AX1] = 0;
            }
            if(pAll_Data->slow.GenVal.ax2_id)
            {
            ...
            {
                pAll_Data->slow.MomtVal[IM_GAS_AX2] = 0;
            }
...
// 				AlarmsWork.presentAlarms &= ~(ALARM_FIAA_HIGH | ALARM_ETAA_HIGH);
// 				AlarmsWork.presentAlarms1 &= ~(ALARM_FIAA_LOW |ALARM_ETAA_LOW);
// #endif
// 			}
...
// 			}
        ...
        case IRMA_SENSOR_REGS:
            ...
            pAll_Data->slow.GenVal.sw_err = SLOW_DATA2 & 0x01; //软件版本错误，重启传感器

            pAll_Data->slow.GenVal.hW_err = SLOW_DATA2 & 0x02;  //硬件版本错误，更换传感器报警

            pAll_Data->slow.GenVal.m_fail = SLOW_DATA2 & 0x04; //电机速度超限

            pAll_Data->slow.GenVal.un_cal = SLOW_DATA2 & 0x08; //出厂校准缺失中

            pAll_Data->slow.GenVal.o2sen_err = SLOW_DATA2 & 0x10;//氧传感器错误

            pAll_Data->slow.GenVal.repl_adapt = SLOW_DATA3 & 0x01; //更换适配器

            pAll_Data->slow.GenVal.no_adapt = SLOW_DATA3 & 0x02; //无适配器，请连接适配器

            // pAll_Data->slow.GenVal.o2_clg = SLOW_DATA3 & 0x04; //氧气端口堵塞

            pAll_Data->slow.GenVal.co2_or = SLOW_DATA4 & 0x01; //二氧化碳超差

            pAll_Data->slow.GenVal.n2o_or = SLOW_DATA4 & 0x02; //一氧化二氮超差

            pAll_Data->slow.GenVal.ax_or = SLOW_DATA4 & 0x04; //至少一种麻醉气体超差

            pAll_Data->slow.GenVal.o2_or = SLOW_DATA4 & 0x08; //氧气超差
            pAll_Data->slow.GenVal.temp_or = SLOW_DATA4 & 0x10; //内部温度高

            pAll_Data->slow.GenVal.press_or = SLOW_DATA4 & 0x20;  //大气压超出运行范围

            pAll_Data->slow.GenVal.zero_or = SLOW_DATA4 & 0x40; //零基准不准确，需要归零

            pAll_Data->slow.GenVal.agen_id_unreliable = SLOW_DATA4 & 0x80; //麻醉气体标识和浓度不可信
...
// 			alarm_str_init(LanguageType); //ylk del 20181128
        ...
        case IRMA_SERVICE_DATA: //frame6: ServiceData
            ...
            pAll_Data->slow.GenVal.zero_disab = SLOW_DATA2 & 0x01;//该位不为零则禁止0点标校
            pAll_Data->slow.GenVal.zero_inprogress = SLOW_DATA2 & 0x02;//该位不为零则零点标校进行中
			pAll_Data->slow.GenVal.span_err = SLOW_DATA2 & 0x04;
			pAll_Data->slow.GenVal.span_inprogress = SLOW_DATA2 & 0x08;
            
            if (pAll_Data->slow.GenVal.zero_inprogress)
            {
              zeroing_state = pAll_Data->slow.GenVal.zero_inprogress;
            }
            if (pAll_Data->slow.GenVal.span_inprogress)
            {
              spaning_state = pAll_Data->slow.GenVal.span_inprogress;
            }
            ...
            pAll_Data->slow.VerVal.serial_num = ((long)SLOW_DATA4<<16) |
                                                ((unsigned int)SLOW_DATA0<<8) | //ls 'int' to 'uint'
                                                SLOW_DATA1;
            break;
    ...
    {
// 		if( (AA_Module_Flag.EtCO2ModuleExist&0x06) != 0) //ylk del 20181128
// 		{
...
//}
void Irma_CreateTxFrame(enum IM_TX_CMD cmd, unsigned char para, unsigned char *ptx_frame)
{
    if (cmd == IM_SET_O2VOL)//氧补偿命令，计算氧补偿值
    {
        para = ConvO2ConcToComp(para);
    ...
    *ptx_frame = IRMA_DATA_FLAG1;//0xAA
    *(ptx_frame+1) = IRMA_DATA_FLAG2;//0x55
    *(ptx_frame+2) = cmd;
    *(ptx_frame+3) = para;
    *(ptx_frame+4) = ~(cmd+para)+1;
}
//函数功能：创建零点标定的命令帧
...
//}
void Irma_CreateNormZeroCalFrame(unsigned char *ptx_frame)
{
    Irma_CreateTxFrame(IM_ZERO_CAL, IM_CAL_NORM, ptx_frame);
}
...
void Irma_CreatePreSpanCalZeroFrame(unsigned char *ptx_frame)
{
...
{
    Irma_setOnOff(IM_OFF);//Irma_OnOff = IM_OFF;
    COMM_TMOUT = 1;
}
/******************************************************************************
  名称：获取CO2模块通讯超时标志
...
// 	case IM_GET_O2_ERR:
// 		return (pAll_Data->slow.GenVal.o2sen_err); //ylk del 20181128
    default:
        return(0);
    }
}
                                  ...
                                  unsigned int *pMT)
{
    ...
    {
        ++ReadFlag;
        *pMT = pAll_Data->slow.VerVal.sen_type; //16b
...
//函数功能：获得零点信息
//输入：void
//返回：char 0：不可用，1：正在校零，2：其他
char Irma_getZeroState(void)
{
    if(pAll_Data->slow.GenVal.zero_disab)
    {
        return 1;
    }
    else if(pAll_Data->slow.GenVal.zero_inprogress)
    {
        return 2;
    }
    else if(pAll_Data->slow.GenVal.zero_or)
    {
        return 3;
    }
    else
...
//函数功能：获得校准信息
//输入：void
//返回：char 0：没有在量程校准过程中，1：在量程校准过程中
...
Int8U PhaseIn_SetOpMode(Int8U opmode,Int8U* tx_buffer)//
{
	*tx_buffer = IRMA_DATA_FLAG1;
	*(tx_buffer+1) = IRMA_DATA_FLAG2;
	*(tx_buffer+2) = IM_SET_MODE;
	*(tx_buffer+3) = opmode;
	*(tx_buffer+4) = ~(IM_SET_MODE + opmode) + 1;

	return 5;
}

Int8U PhaseIn_SpanCal(Int8U type,Int8U data,Int8U* tx_buffer)
{
	*tx_buffer = IRMA_DATA_FLAG1;
	*(tx_buffer+1) = IRMA_DATA_FLAG2;
	*(tx_buffer+2) = type;
	*(tx_buffer+3) = data;
	*(tx_buffer+4) = ~(type + data) + 1;

	return 5;
}

Int8U PhaseIn_SetIRO2Delay(Int8U data,Int8U* tx_buffer)
{
	*tx_buffer = IRMA_DATA_FLAG1;
	*(tx_buffer+1) = IRMA_DATA_FLAG2;
	*(tx_buffer+2) = IM_SET_IRO2_DELAY;
	*(tx_buffer+3) = data;
	*(tx_buffer+4) = ~(IM_SET_IRO2_DELAY + data) + 1;

	return 5;
}

Int8U PhaseIn_SetPump(Int8U pump,Int8U* tx_buffer)
{
	*tx_buffer = IRMA_DATA_FLAG1;
	*(tx_buffer+1) = IRMA_DATA_FLAG2;
	*(tx_buffer+2) = IM_SET_PUMP;
	*(tx_buffer+3) = pump;
	*(tx_buffer+4) = ~(IM_SET_PUMP + pump) + 1;

	return 5;
}

Int8U PhaseIn_SetO2Option(Int8U mode,Int8U* tx_buffer)
{
	*tx_buffer = IRMA_DATA_FLAG1;
	*(tx_buffer+1) = IRMA_DATA_FLAG2;
	*(tx_buffer+2) = IM_SETO2_OPTION;
	*(tx_buffer+3) = mode;
	*(tx_buffer+4) = ~(IM_SETO2_OPTION + mode) + 1;

	return 5;
}

Int8U PhaseIn_SetO2SensorType(Int8U Type,Int8U* tx_buffer)
{
	*tx_buffer = IRMA_DATA_FLAG1;
	*(tx_buffer+1) = IRMA_DATA_FLAG2;
	*(tx_buffer+2) = IM_SETO2_TYPE;
	*(tx_buffer+3) = Type;
	*(tx_buffer+4) = ~(IM_SETO2_TYPE + Type) + 1;

	return 5;
}

// void SetISA_O2Option(unsigned char option) //ylk del 20181128
// {
// 	ISAO2_Type.O2Option = option;
// }
//
// void SetISA_O2Type(unsigned char Type)
// {
// 	ISAO2_Type.O2SenType = Type;
// }
//
// void SetISA_O2IRDelay(unsigned char IRDelay)
// {
// 	ISAO2_Type.O2IRDelay = IRDelay;
// }

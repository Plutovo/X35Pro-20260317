...
unsigned int Phasein_RunFlag = 0;//djq add 20161209临时增加变量，用于检测模块和GUI持续通讯的，如果通讯断开，则清空PhaseIn参数,验证完成后加入结构体中
...
//        CHKSUM = 0;//djq add 2013-12-23
        if( Masimo_EtCO2_Config_On && ((EtCO2ModuleExist&0x02) != 2) )//选配开通 未置位
        {
            EtCO2ModuleExist |= 1<<1;   //djq add 2013-12-23 第二位置1
        }
		else if( MasimoISA_EtCO2_Config_On && ((EtCO2ModuleExist&0x04) != 4) )//选配开通 未置位
		{
			EtCO2ModuleExist |= 1<<2;   //djq add 2013-12-23 第二位置1
		}
        if(CHKSUM==0) //Data OK
        {
            //获取模块状态
    ...
    while (IM_RX_HEAD != IM_RX_TAIL)
    {
        ...
        case IRMA_GEN_VALS: //frame3: GenVals
            ...
            case AX_Hal:
                ...
                strcpy(pAll_Data->slow.GenVal.ax1_type, System_Dispaly[ STR_HAL ]);//氟烷
                break;
            case AX_Enf:
                pAll_Data->slow.GenVal.ax1_color = ENFCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax1_type, System_Dispaly[ STR_ENF ]);//安氟醚
                break;
            case AX_Iso:
                pAll_Data->slow.GenVal.ax1_color = ISOCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax1_type, System_Dispaly[ STR_ISO ]);//异氟醚
                break;
            case AX_Sev:
                pAll_Data->slow.GenVal.ax1_color = SEVCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax1_type, System_Dispaly[ STR_SEV ]);//七氟醚
                break;
            case AX_Des:  //sun add 20140605
                pAll_Data->slow.GenVal.ax1_color = DESCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax1_type, System_Dispaly[ STR_DES ]);//地氟醚
                break;
            default:
            ...
            case AX_Hal:
                ...
                strcpy(pAll_Data->slow.GenVal.ax2_type, (char *)System_Dispaly[ STR_HAL ]);//氟烷
                break;
            case AX_Enf:
                pAll_Data->slow.GenVal.ax2_color = ENFCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax2_type, (char *)System_Dispaly[ STR_ENF ]);//安氟醚
                break;
            case AX_Iso:
                pAll_Data->slow.GenVal.ax2_color = ISOCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax2_type, (char *)System_Dispaly[ STR_ISO ]);//异氟醚
                break;
            case AX_Sev:
                pAll_Data->slow.GenVal.ax2_color = SEVCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax2_type, (char *)System_Dispaly[ STR_SEV ]);//七氟醚
                break;
            case AX_Des:  //sun add 20140605
                pAll_Data->slow.GenVal.ax2_color = DESCOLOR;
                strcpy(pAll_Data->slow.GenVal.ax2_type, (char *)System_Dispaly[ STR_DES ]);//地氟醚
                break;
            default:
        ...
        case IRMA_SERVICE_DATA: //frame6: ServiceData
            ...
            pAll_Data->slow.GenVal.span_err = SLOW_DATA2 & 0x04;//ISA最新的量程校准命令失败
            pAll_Data->slow.GenVal.span_inprogress = SLOW_DATA2 & 0x08;//正在进行量程校准和验证
...
enum IM_FUNC_STS Irma_RX_Process(void)
{
    ...
    static unsigned int Old_Flag = 0;
    static unsigned int DisConnectTime = 0;//模块断开持续时间
    static unsigned int ConnectTime = 0;   //模块连接持续时间
    static unsigned char RunFlag_Cnt = 0;
    switch(Irma_getOnOff())
    {
...
//            return(pAll_Data->slow.VerVal.rid6_byte5);
	...
	case IM_GET_O2_ERR:
		return (pAll_Data->slow.GenVal.o2sen_err);
    default:
        return(0);
    }
}
...
void SetISA_O2Option(unsigned char option)
{
...
void Module_Alarm_Judge(void)  //sun add 20140821
{
	...
	else//masimo 旁流
	{
		...
		if(pAll_Data->slow.GenVal.no_adapt!=0)
		{
			if(!(AlarmsWork.presentAlarms & ALARM_PHASEIN_NO_TUBE))
			{
				AlarmsWork.newAlarms |= ALARM_PHASEIN_NO_TUBE;
			}
		}
		else
		{
			AlarmsWork.presentAlarms &= ~ALARM_PHASEIN_NO_TUBE;
		}
		if(pAll_Data->slow.GenVal.O2_Calib)
		{
			if(!(AlarmsWork.presentAlarms & ALARM_PHASEIN_O2CALIB))
			{
				AlarmsWork.newAlarms |= ALARM_PHASEIN_O2CALIB;
			}
		}
		else
		{
			AlarmsWork.presentAlarms &= ~ALARM_PHASEIN_O2CALIB;
		}
	}
	...
	if(pAll_Data->slow.GenVal.o2_or!=0)
	{
		if(!(AlarmsWork.presentAlarms & ALARM_PHASEIN_O2_OR))
		{                
			AlarmsWork.newAlarms |= ALARM_PHASEIN_O2_OR;
		}
	}
	else
	{
		AlarmsWork.presentAlarms &= ~ALARM_PHASEIN_O2_OR;
	}

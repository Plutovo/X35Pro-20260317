...
----------------------------------------------------------------------------------------------------
File        : BUTTON_MENU.c
Purpose     : Implementation of buttonMenu widget
----------------------------------------------------------------------------------------------------
*/
#include <stdlib.h>
#include <string.h>
#include "BUTTON_MENU_Private.h"
...
#define BUTTON_FONT_DEFAULT &GUI_FontHZ_SimSun_16
#endif
...
BUTTON_MENU_PROPS BUTTON_MENU__DefaultProps =
{
...
*       _Paint
*/
static void _Paint(BUTTON_MENU_Obj* pObj, BUTTON_MENU_Handle hObj)
{
    const char* s = NULL;
    unsigned int Index;
    ...
    PressedState = (State & BUTTON_MENU_STATE_PRESSED) ? 1 : 0;
    ...
    */
    if (ColorIndex < 2)
    {
        Index = (pObj->ahDrawObj[BUTTON_MENU_BI_PRESSED] && PressedState) ? BUTTON_MENU_BI_PRESSED : BUTTON_MENU_BI_UNPRESSED;
    }
    else
    {
        Index = pObj->ahDrawObj[BUTTON_MENU_BI_DISABLED] ? BUTTON_MENU_BI_DISABLED : BUTTON_MENU_BI_UNPRESSED;
    }
...
*       _RoundRectPaint
*/
static void _RoundRectPaint(BUTTON_MENU_Obj* pObj, BUTTON_MENU_Handle hObj)
{
    const char* s = NULL;
    unsigned int Index;
    int State, PressedState, ColorIndex;
    ...
    */
    ...
    /* Draw focus */
    if (State & BUTTON_MENU_STATE_FOCUS)
    {
        LCD_SetColor(GUI_BLUE);
        GUI_DrawFocusRect(&rClient, 3);
        GUI_DrawFocusRect(&rClient, 2);
...
*/
static void _Delete(BUTTON_MENU_Obj* pObj)
{
...
*       _ButtonPressed
*/
static void _ButtonMenuPressed(BUTTON_MENU_Handle hObj, BUTTON_MENU_Obj* pObj)
{
    WIDGET_OrState(hObj, BUTTON_MENU_STATE_PRESSED);
...
*       _OnTouch
*/
static void _OnTouch(BUTTON_MENU_Handle hObj, BUTTON_MENU_Obj* pObj, WM_MESSAGE*pMsg)
{
    ...
    if (pMsg->Data.p)    /* Something happened in our area (pressed or released) */
    {
        if (pState->Pressed)
        {
            if ((pObj->Widget.State & BUTTON_MENU_STATE_PRESSED) == 0)
            {
                _ButtonMenuPressed(hObj, pObj);
            }
        }
        else
        {
            /* React only if button was pressed before ... 
               avoid problems with moving / hiding windows above (such as dropdown) */
            if (pObj->Widget.State & BUTTON_MENU_STATE_PRESSED)
            {
...
{
    BUTTON_MENU_Handle hObj = pMsg->hWin;
    BUTTON_MENU_Obj* pObj = BUTTON_MENU_H2P(hObj);
...
*       BUTTON_CreateEx
*/
BUTTON_MENU_Handle BUTTON_MENU_CreateEx(int x0, int y0, int xsize, int ysize, WM_HWIN hParent, int WinFlags, int ExFlags, int Id)
{
    BUTTON_MENU_Handle hObj;
    GUI_USE_PARA(ExFlags);
    /* Create the window */
    WM_LOCK();
    hObj = WM_CreateWindowAsChild(x0, y0, xsize, ysize, hParent, WinFlags, BUTTON_MENU_Callback,
                                  sizeof(BUTTON_MENU_Obj) - sizeof(WM_Obj));
    if (hObj)
    {
        BUTTON_MENU_Obj* pObj = BUTTON_MENU_H2P(hObj);
        ...
        BUTTON_MENU_INIT_ID(pObj);
        pObj->Props = BUTTON_MENU__DefaultProps;
        pObj->Button_Menu_Use_3D = 1;
    }
    else
    {
...
void BUTTON_MENU_SetText(BUTTON_MENU_Handle hObj, const char* s)
{
    if (hObj)
    {
        BUTTON_MENU_Obj* pObj;
        WM_LOCK();
        pObj = BUTTON_MENU_H2P(hObj);
        if (GUI__SetText(&pObj->hpText, s))
        {
            BUTTON_MENU_Invalidate(hObj);
        }
        WM_UNLOCK();
    }
}
...
void BUTTON_MENU_SetFont(BUTTON_MENU_Handle hObj, const GUI_FONT GUI_UNI_PTR * pfont)
{
    if (hObj)
    {
        ...
        WM_LOCK();
        pObj = BUTTON_MENU_H2P(hObj);
        BUTTON_MENU_ASSERT_IS_VALID_PTR(pObj);
...
*       BUTTON_SetBkColor
*/
void BUTTON_MENU_SetUse3D(BUTTON_MENU_Handle hObj,unsigned int Index)
{
    if (hObj)
    {
        BUTTON_MENU_Obj* pObj;
...
*       BUTTON_SetState
*/
void BUTTON_MENU_SetState(BUTTON_MENU_Handle hObj, int State)
{
...
void BUTTON_MENU_SetPressed(BUTTON_MENU_Handle hObj, int State)
{
    ...
    {
        WIDGET_AndState(hObj, BUTTON_MENU_STATE_PRESSED);
    }
}
...
*       BUTTON_Create
*/
BUTTON_MENU_Handle BUTTON_MENU_Create(int x0, int y0, int xsize, int ysize, int Id, int Flags)
{
    return BUTTON_MENU_CreateEx(x0, y0, xsize, ysize, WM_HMEM_NULL, Flags, 0, Id);
}
...
{
    return BUTTON_MENU_CreateEx(x0, y0, xsize, ysize, hParent, Flags, 0, Id);
}
...
{
    BUTTON_MENU_Handle  hThis;
    GUI_USE_PARA(cb);
    hThis = BUTTON_MENU_CreateEx(pCreateInfo->x0 + x0, pCreateInfo->y0 + y0,
                                 pCreateInfo->xSize, pCreateInfo->ySize,
                                 hWinParent, 0, pCreateInfo->Flags, pCreateInfo->Id);
    BUTTON_MENU_SetText(hThis, pCreateInfo->pName);
    return hThis;
}
#else                            /* Avoid problems with empty object modules */
void BUTTON__MENU_C(void) {}
